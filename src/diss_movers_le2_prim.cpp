    ...
    for(int i=2;i<=ib;i++){
        for(int j=2;j<=jd1;j++){
           ...
           // area_avg = 0.5*(area[i][jm1]+area[i][j]);
            ...
            fd[0] = 0.5*fabs(mdiss[0]+MinMod(duj[0][i][jp1], duj[0][i][j], duj[0][i][jm1])*(std::max(Vnr,Vnl)-mdiss[0]))*(Ur[0]-Ul[0]);
            fd[1] = 0.5*fabs(mdiss[1]+MinMod(duj[1][i][jp1], duj[1][i][j], duj[1][i][jm1])*(std::max(Vnr,Vnl)-mdiss[1]))*(Ur[1]-Ul[1]);
            fd[2] = 0.5*fabs(mdiss[2]+MinMod(duj[2][i][jp1], duj[2][i][j], duj[2][i][jm1])*(std::max(Vnr,Vnl)-mdiss[2]))*(Ur[2]-Ul[2]);
