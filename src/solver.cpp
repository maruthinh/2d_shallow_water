...
void Solver(int ib, int id1, int id2, int jb, int jd1, int jd2, int nconv, int ndepv, double **&x, double **&y, double ***&cv, double ***&dv, double ***&dui, double ***&duj, double **&area, double ***&si, double ***&sj, double **&tstep, double **&sri,  double **&srj, double ***&gradfi,  double ***&tgradfj, double ***&cvold, double ***&diss, double ***&rhs, double ***&epsij){
    ...
    for(int j=2;j<=jb;j++){
        for(int i=2;i<=ib;i++){
            ...
            cv[0][i][j] = (3.0/4.0)*cvold[0][i][j] + (1.0/4.0)*cv[0][i][j] - (1.0/4.0)*rhs[0][i][j];
            cv[1][i][j] = (3.0/4.0)*cvold[1][i][j] + (1.0/4.0)*cv[1][i][j] - (1.0/4.0)*rhs[1][i][j];
            cv[2][i][j] = (3.0/4.0)*cvold[2][i][j] + (1.0/4.0)*cv[2][i][j] - (1.0/4.0)*rhs[2][i][j];
        }
    }
    ...
    for(int j=2;j<=jb;j++){
        for(int i=2;i<=ib;i++){
            ...
            cv[0][i][j] = (1.0/3.0)*cvold[0][i][j] + (2.0/3.0)*cv[0][i][j] - (2.0/3.0)*rhs[0][i][j];
            cv[1][i][j] = (1.0/3.0)*cvold[1][i][j] + (2.0/3.0)*cv[1][i][j] - (2.0/3.0)*rhs[1][i][j];
            cv[2][i][j] = (1.0/3.0)*cvold[2][i][j] + (2.0/3.0)*cv[2][i][j] - (2.0/3.0)*rhs[2][i][j];
        }
    }
